...
<div class="l-signup-container l-prof-edit-container">
  <form class="p-signup" method="post" enctype="multipart/form-data">
    ...
    <label>名前
      <span class="u-err-msg <?php if(!empty($err_msg['name'])) echo 'c-err-msg';?>"><?php if(!empty($err_msg['name'])) echo $err_msg['name'];?></span>
      <input type="text" name="name" class="c-input p-signup__form <?php if(!empty($err_msg['name'])) echo 'u-err-border';?>" value="<?php echo profInputHold('name');?>">
    </label>
    
    <label>郵便番号
      <span class="u-err-msg <?php if(!empty($err_msg['zip'])) echo 'c-err-msg';?>"><?php if(!empty($err_msg['zip'])) echo $err_msg['zip'];?></span>
      <input type="text" name="zip" class="c-input p-signup__form <?php if(!empty($err_msg['zip'])) echo 'u-err-border';?>" value="<?php echo profInputHold('zip');?>">
    </label>

    <div class="p-prof-upload">
      <img src="<?php if(!empty($dbPic1)) echo $dbPic1;?>" class="p-prof-upload__img js-show-prof-img">
      <label for="up-file">ファイルを選択する
        <input id="up-file" type="file" name="pic1" class="p-prof-upload__input js-up-prof">
      </label>
    </div>

    <input type="submit" class="c-bt p-signup__bt" value="編集する">
  </form>
</div>

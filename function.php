...
define('MSG06','確認用のパスワードが新しいパスワードと一致しません。');
define('MSG07','こちらのメールアドレスはすでに登録されています。');
define('MSG08','エラーが発生しました。');
define('MSG09','郵便番号の形式が違います。');
define('MSG10','古いパスワードが違います。');
define('MSG11','新しいパスワードが現在のパスワードと変わっていません。');
define('MSG12','メールアドレスまたはパスワードが違います。');
...
function getMessageList($currentMinNum=1,$tag,$span){
  try{
    ...
    $sql .= ' ORDER BY create_date DESC'. ' LIMIT '. $span. ' OFFSET '. $currentMinNum;
...
function searchMessage($currentMinNum,$search_msg,$span){
  try{
    ...
    $sql = 'SELECT msg,create_date,msg_id,user_id FROM message WHERE msg=:search_msg';
